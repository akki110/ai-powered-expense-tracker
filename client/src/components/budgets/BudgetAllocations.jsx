  ...
  return (
    <div className="flex flex-col space-y-6">
      ...
      <div className="flex flex-col space-y-4">
        {loading ? (
          <div className="text-center p-8 text-muted-foreground border border-zinc-200 dark:border-zinc-800 rounded-sm">
            <Loader />
          </div>
        ) : budgets.length === 0 ? (
          <div className="text-center p-8 text-muted-foreground border border-zinc-200 dark:border-zinc-800 rounded-sm">
          ...
          budgets.map((budget) => {
            ...
            return (
              <div
                key={budget._id}
                className="border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-6 rounded-sm shadow-sm hover:shadow-md transition-shadow relative"
              >
                <Button
                  variant="ghost"
                  size="icon"
                ...
                <div className="flex justify-between items-start mb-4 pr-10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-zinc-900 dark:text-white tracking-wide text-base uppercase">
                        {budget.category}
                      </h3>
                  ...
                  <div className="text-right">
                    <div className="font-bold text-zinc-900 dark:text-white tracking-wide">
                      <span
                        className={
                          progress > 90 ? "text-red-600 dark:text-red-500" : ""
                        }
                      >
                        $
                      ...
                      >
                        $
                        {(budget.limit || 0).toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                        })}
                      </span>
                    </div>
